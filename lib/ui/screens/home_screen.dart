...
class HomeScreen extends StatelessWidget {
  ...
  HomeScreen(this.miUsuario,{super.key});
    ...
    return Scaffold(
        ...
        drawer: Drawer(
          ...
          children: [
            ...
            ListTile(leading: const CircleAvatar(child: Icon(Icons.login), ),title: const Text("Salir"),subtitle: const Text('Presiona para salir'),onTap: () 
            {
        ...
        body: Column(
          children: [
            HeaderCard(miUsuario: miUsuario, convertedDateTime: convertedDateTime),
            if (miUsuario.name.isNotEmpty) DataCard(miUsuario.name, Icons.person),
            if (miUsuario.phone.isNotEmpty) DataCard(miUsuario.phone, Icons.phone),
...
class DataCard extends StatelessWidget
{
  ...
  Widget build(BuildContext context) 
  {
    return Card(margin: const EdgeInsets.all(4.0),color: Colors.blue,
      shadowColor: const Color(0xff000000),
      elevation: 1,
      shape: RoundedRectangleBorder(
        borderRadius: BorderRadius.circular(4.0),
        side: const BorderSide(color: Color(0x4d9e9e9e), width: 1),
      ),child: Container(margin: const EdgeInsets.all(10),  child: Row(children: [Icon(icon, ), Text(info), ])),
      );
  }
}
